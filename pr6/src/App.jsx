...
function App() {
  ...
  const handleSort = (name, filter) => {

    let records = users;

    if (filter === 'up' && name === 'fname') {

      let ascendingData = records.sort((a, b) => {
        return a.firstName.localeCompare(b.firstName);
      });
      setData(ascendingData)

    } else if (filter == 'dn' && name === 'fname') {

      let dscendingData = records.sort((a, b) => {
        return b.firstName.localeCompare(a.firstName);
      });
      setData(dscendingData)

    } else if (filter === 'up' && name === 'email') {
    ...
    } else if (filter === 'up' && name === 'username') {
    ...
    } else if (filter == 'dn' && name === 'username') {
  ...
  const handleChange = (e) => {
    ...
    let records = users;
    let searchData = records.filter((record) => {
  ...
  const handleSearch = (e) => {
    ...
    if (search == '') {
      ...
      let records = users;
      let searchData = records.filter((record) => {
        return search.toLowerCase() === record.firstName.toLowerCase();
      });
      setData(searchData);
  ...
  const handleCity = (city) => {
    ...
    if (city === 'all') {
      ...
      let cityData = records.filter((record) => {
        ...
        <Container>
          <Row>
            ...
            <div className="mt-4 d-flex flex-wrap justify-content-center">
              <form className="w-50 d-flex flex-wrap btns" onSubmit={(e) => handleSearch(e)}>
                <input type="text" className="form-control w-75" placeholder="Enter Search..." name="search" value={search} onChange={(e) => handleChange(e)} />
                <button className="ms-3 border rounded-circle bg-white" type="submit">&#128269;</button>
                <button className="ms-3 px-2 text-black border rounded-circle bg-white" type="button" onClick={handleRefresh}>&#x21bb;</button>
              </form>
              <Dropdown>
                <Dropdown.Toggle variant="info">
                  Citys
                </Dropdown.Toggle>
                <Dropdown.Menu className="bg-info" style={{ height: 400, overflowY: 'scroll' }}>
                  <Dropdown.Item href="#/action-1" onClick={() => handleCity('all')}>All</Dropdown.Item>
                  {users.map((el) => {
                    return (<Dropdown.Item href="#/action-1" onClick={() => handleCity(el.address.city)}>{
                      el.address.city
                    }</Dropdown.Item>)
                  })}
                </Dropdown.Menu>
            ...
            <Table bordered className="my-5 border-black">
              <thead>
                <tr>
                  <th>#</th>
                  <th>Full Name<button className="ms-5" onClick={() => handleSort('fname', 'up')}>&#x2B06;</button><button onClick={() => handleSort('fname', 'dn')}>&#x2B07;</button></th>
                  <th>Age</th>
                  <th>User Name<button className="ms-5" onClick={() => handleSort('username', 'up')}>&#x2B06;</button><button onClick={() => handleSort('username', 'dn')}>&#x2B07;</button></th>
                  <th>Email<button className="ms-5" onClick={() => handleSort('email', 'up')}>&#x2B06;</button><button onClick={() => handleSort('email', 'dn')}>&#x2B07;</button></th>
                  <th>Contact</th>
                  <th>Address</th>
                </tr>
              </thead>
              <tbody>
                {
                  data.map((el) => {
                    return (
                      <tr>
                        <td>{el.id}</td>
                        <td>{el.firstName} {el.lastName} {el.maidenName}</td>
                        <td>{el.age}</td>
                        <td>{el.username}</td>
                        <td>{el.email}</td>
                        <td>{el.phone}</td>
                        <td>{el.address.address} , {el.address.city}</td>
                      </tr>
